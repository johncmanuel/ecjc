...
export default function EntryCard({
	...
	const [localReactions, setLocalReactions] = useState<ReactionResponse[]>(
		reactions || [],
	);
	...
	const handleToggleReaction = async (emojiCode: string) => {
		const hasReacted = localReactions.some(
			(r) => r.emojiCode === emojiCode && r.userId === currentUserId,
		);
		...
		if (hasReacted) {
			...
			try {
				...
				setLocalReactions((prev) => [
					...prev,
					{ emojiCode, userId: currentUserId },
				]);
			}
		} else {
			const optimisticReaction: ReactionResponse = {
			...
			try {
				...
				setLocalReactions((prev) =>
					prev.map((r) => (r === optimisticReaction ? response : r)),
				);
			} catch (e) {
				console.error("Failed to add reaction", e);
				setLocalReactions((prev) =>
					prev.filter((r) => r !== optimisticReaction),
				);
			}
		}
		setShowEmojiPicker(false);
