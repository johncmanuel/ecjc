...
export default function MediaPreview({
	...
	return (
		<div
			className={`mt-4.5 grid gap-2 ${files.length > 1 ? "grid-cols-2" : "grid-cols-1"}`}
		>
			{files.map((file, index) => {
				...
				return (
					<div
						key={file.name + index}
						className="relative h-36 rounded-2xl bg-paper-deep border border-line overflow-hidden"
					>
						{isVideo ? (
							<video
								src={previews[index]}
