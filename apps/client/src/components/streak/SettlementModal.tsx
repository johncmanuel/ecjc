...
export default function SettlementModal({
	...
	const dividedDebt =
		members.length > 0 ? (accumulatedDebt / members.length).toFixed(2) : "0.00";
			...
			<div className="relative w-full max-w-sm bg-paper rounded-xl shadow-xl overflow-hidden mx-4 flex flex-col">
				...
				<div className="flex-1 px-6 pt-2 pb-6 flex flex-col items-center text-center">
					...
					<p className="text-[12.5px] text-ink-soft leading-relaxed max-w-[240px] mb-6.5">
						You owe a total of ${accumulatedDebt.toFixed(2)}. Pay your friends
							...
							{members.length === 0 ? (
								...
								members.map((member) => {
									...
									return (
										<div
											key={member.userId}
											className="p-4 bg-card border border-line rounded-xl text-left"
										>
											...
											{!hasAnyHandle ? (
												<p className="text-[11.5px] text-ink-faint bg-paper p-2 rounded border border-line">
													This user hasn't configured any payment handles yet.
													You'll have to ask them where to send the cash!
												</p>
											) : (
