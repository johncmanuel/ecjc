...
export default function InvitePage() {
	...
	const handleJoin: React.FormEventHandler<HTMLFormElement> = async (e) => {
		...
		try {
			...
			console.error(err);
			setError(
				err?.message || "Failed to send invite. Please check the friend code.",
			);
		} finally {
			setIsJoining(false);
		}
					...
					<div className="space-y-4 pt-6 border-t border-line">
						...
						<div className="space-y-3">
							{sentInvites.map((invite) => (
								<div
									key={invite.id}
									className="flex items-center justify-between p-4 bg-paper-deep rounded-xl border border-line"
								>
