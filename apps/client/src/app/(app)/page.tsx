...
export default function TimelinePage() {
	...
	if (isLoading || !activeGroup) {
		return (
			<div className="flex justify-center items-center h-64 text-ink-soft">
				Loading timeline...
			</div>
		);
	}
		...
		<main className="pb-24">
				...
				style={{
					transform:
						scrollDirection === "down"
							? "translateY(calc(-100% - 69px))"
							: "translateY(0)",
				}}
			>
				<div className="flex items-baseline justify-center gap-1.5">
					<span className="font-serif text-xl font-medium">
						{activeGroup.streakCount}
					</span>
					<span className="text-sm text-ink-soft">
						days sharing something, together
					</span>
				</div>
			...
			{hasLeftMember && (
				<div className="bg-amber-50 dark:bg-amber-950/30 border-b border-amber-200 dark:border-amber-900 px-4 py-3 text-sm text-amber-900 dark:text-amber-200 flex flex-col gap-2">
					<p>
						Your partner has left this group. This timeline is now read-only.
					</p>
					<button
				...
				<div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm">
					<div className="bg-paper dark:bg-card border border-line rounded-2xl p-6 max-w-sm w-full shadow-xl">
						...
						<p className="text-sm text-ink-soft mb-6">
							Are you sure you want to leave this group? The data will remain
							for your partner, but you will lose access.
						</p>
