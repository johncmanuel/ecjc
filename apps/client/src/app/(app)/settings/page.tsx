...
export default function SettingsPage() {
	...
	const accumulatedPenalty = activeGroup?.accumulatedPenaltyCents
		? activeGroup.accumulatedPenaltyCents / 100
		: 0;
			...
			try {
				...
				if (profileRes) {
					...
					setEditName(
						profileRes.firstName
							? `${profileRes.firstName} ${profileRes.lastName}`.trim()
							: "",
					);
				...
				if (penaltyRes) {
					...
					if (penaltyRes.penaltyAmountCents) {
						setPenaltyAmount(
							Math.max(1, Math.round(penaltyRes.penaltyAmountCents / 100)),
						);
					}
				}
			} catch (e) {
	...
	const handleTogglePledge = async (v: boolean) => {
		setMoneyPledge(v);
		try {
			await api.postApiSettingsPenalty({
				isPenaltyEnabled: v,
				penaltyAmountCents: penaltyAmount * 100,
			});
		} catch (e) {
			console.error(e);
	...
	const handleChangePenaltyAmount = useCallback(
		(newAmount: number) => {
			...
			penaltySaveTimeout.current = setTimeout(async () => {
				try {
					await api.postApiSettingsPenalty({
						isPenaltyEnabled: moneyPledge,
						penaltyAmountCents: newAmount * 100,
					});
				} catch (e) {
	...
	return (
		<div className="pb-20">
			<h1 className="font-serif text-3xl font-medium px-5 pt-8 pb-2 text-ink">
				Settings
			</h1>
			...
			<div className="px-4.5 mt-5 space-y-10">
							...
							<div className="space-y-4">
								...
								<div className="pt-2 border-t border-line mt-4">
									<p className="text-xs text-ink-faint mb-3">
										Add your handles to easily receive penalties from friends.
									</p>
							...
							<div>
								...
								<div className="pt-3 border-t border-line text-sm text-ink-faint">
									<div className="grid grid-cols-3 gap-2">
										<div>
											<span className="block text-[10px] uppercase font-medium mb-0.5">
												Venmo
											</span>
											<span className="text-ink">
												{profile?.venmoHandle || "Not set"}
											</span>
										</div>
										<div>
											<span className="block text-[10px] uppercase font-medium mb-0.5">
				...
				<section>
					...
					<SettingsRow
						...
						description={
							moneyPledge
								? `You are committed to paying $${penaltyAmount} if you miss a day`
								: `Enable to owe $${penaltyAmount} to your friends if you fail`
						}
						on={moneyPledge}
					...
					{moneyPledge && (
						<div className="px-3 mt-4 flex items-center justify-between">
							...
							<AmountStepper
								...
								onChange={handleChangePenaltyAmount}
							/>
						</div>
					)}
					...
					<div className="bg-card border border-line rounded-xl p-5 space-y-6">
						<div>
							...
							<p className="text-sm text-ink-faint leading-relaxed">
								Tokens you generate can be used to authenticate with the ECJC
								API. These tokens give an agent full access to your account
								data.
							</p>
						</div>
						...
						{generatedToken && (
							<div className="bg-green-500/10 border border-green-500/20 rounded-lg p-4 mb-4">
								<p className="text-sm text-green-700 font-medium mb-2">
									Key generated successfully! Copy this token now. You will not
							...
							<div className="space-y-3">
								{apiKeys.map((key) => {
									...
									return (
										<div
											key={key.id}
											className="flex items-start justify-between p-3 bg-paper border border-line rounded-lg gap-2"
										>
											<div className="flex-1">
												<div className="flex items-center space-x-2">
													<p
														className={`text-sm font-medium ${isExpired ? "text-ink-faint line-through" : "text-ink"}`}
													>
														{key.name}
													</p>
													{isExpired && (
												...
												<div className="flex items-center flex-wrap gap-x-3 gap-y-1 mt-1 text-xs text-ink-faint">
													<code className="px-1.5 py-0.5 bg-card border border-line rounded font-mono shrink-0">
														{key.prefix}
													</code>
